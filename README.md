
<h1>Hi, I'm Josué Ríos! </h1>
<h3>
Engineer | DataSecOps | Digital Energy Transformation
</h3>



<h2>⚡ About Me</h2>

Electrical & Systems Engineer specializing in **Smart Grids, Data Science, and Cybersecurity**. Passionate about transforming the IT, OT, and Energy sector through data-driven solutions and secure digitalization.

<h2>🎯 Professional Focus</h2>

- Predictive maintenance for energy infrastructure.
- Secure AIoT implementation for smart grids.
- Data analytics for renewable energy optimization.
- OT/IT cybersecurity in critical infrastructure.

<h2>🔧 Technical Projects Portfolio</h2>

| Category | Technologies |
|----------|-------------|
| **Programming** | Python, SQL, R, C++, C |
| **Cloud Platforms** | GCP, AWS, Microsoft Azure, GitHub |
| **Cybersecurity** | NIST CSF, MITRE ATT&CK, ISO 27001, CIS Controls |
| **Data Science** | Pandas, Scikit-learn, Power BI, Tableau |
| **Automation** | Ansible, Bash, PowerShell |
| **Operating Systems** | Linux, Windows |
| **Energy/OT Systems** | SCADA, Grid Operations, DER Integration, Smart Grid |


### 🚀 Currently
- Advancing skills in Cloud Computing, Cybersecurity & Applied Data Science.
- Exploring Machine Learning applications for energy optimization.
- Contributing to secure digital transformation in the energy sector.

---

### 📍 Location & Availability
**Based:** Colombia | **Relocation:** Yes | **Remote:** Yes  
**Languages:** Spanish (Native), English (C1 - B2 Certified), Portuguese (B1).


## 📫 Let's Connect
- 📧 Email: engrjosuerios@gmail.com.
- 💼 LinkedIn: <a href="http://www.linkedin.com/in/engr-josue-rios/">Engr. Josué Ríos C</a>.
- 🖥️ LeetCode: <a href="https://leetcode.com/u/engrjosuerios/">engrjosuerios</a>.
